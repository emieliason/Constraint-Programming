...
problem9(Grid):-
  ...
  A1sq = A1 + A2 + A3 + A4 + B1 + C1 + D1,
  A2sq = A1 + A2 + A3 + A4 + B2 + C2 + D2,
  A3sq = A1 + A2 + A3 + A4 + B3 + C3 + D3,
  A4sq = A1 + A2 + A3 + A4 + B4 + C4 + D4,
  
  B1sq = B1 + B2 + B3 + B4 + A1 + C1 + D1,
  B2sq = B1 + B2 + B3 + B4 + A2 + C2 + D2,
  B3sq = B1 + B2 + B3 + B4 + A3 + C3 + D3,
  B4sq = B1 + B2 + B3 + B4 + A4 + C4 + D4,
  
  C1sq = C1 + C2 + C3 + C4 + A1 + B1 + D1,
  C2sq = C1 + C2 + C3 + C4 + A2 + B2 + D2,
  C3sq = C1 + C2 + C3 + C4 + A3 + B3 + D3,
  C4sq = C1 + C2 + C3 + C4 + A4 + B4 + D4,

  D1sq = D1 + D2 + D3 + D4 + A1 + B1 + C1,
  D2sq = D1 + D2 + D3 + D4 + A2 + B2 + C2,
  D3sq = D1 + D2 + D3 + D4 + A3 + B3 + C3,
  D4sq = D1 + D2 + D3 + D4 + A4 + B4 + C4,

/*

If we start with fully unlit field (right half of the handout pic), then just constraint A1sq .... D4sq to be odd.
If we start with some squares lit (left half of the handout pic), then change the sums:
  If A2 is already lit, then A1sq =  A1 + 1 + A3 + A4 + B1 + C1 + D1
  I.e., '1' instead of 'A2'.
  Unless we are allowed to clik the squares that are initially lit.
  His "hit" implies we are not, but the problem write-up is vague.
